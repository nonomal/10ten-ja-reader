...
export class TopWindowFinder {
  ...
  constructor(private postMessageProxy: PostMessageProxy) {
    ...
    this.postMessageProxy.addListener((event) => {
      if (isObject(event) && isMessageSourceWindow(event.source)) {
        if (event.data === '10ten(ja):ping') {
          ...
          this.topMostWindow = event.source;
    ...
    requestIdleCallback(
      () => {
        this.lookupTopMostWindow();
      },
      { timeout: 4000 }
    );
  }

  public getTopMostWindow(): Window {
    return this.topMostWindow;
  }

  public isTopMostWindow(): boolean {
    return window.self === this.getTopMostWindow();
  }

  private async lookupTopMostWindow() {
    // Common case: if we are the topmost window then we know for absolute
    // certain the content script is running there!
    if (window.self === window.top) {
      this.topMostWindow = window.self;
      return;
    }
    ...
    try {
      if (window.location.origin === window.top?.location.origin) {
        this.topMostWindow = window.top;
        return;
      }
    } catch {
      // Ignore: We likely got a security exception because the topmost window
      // is cross-origin.
    }
